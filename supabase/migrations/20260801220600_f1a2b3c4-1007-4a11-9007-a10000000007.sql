...
-- ============ FIX F-02 (BR-11): DOUBLE-DECISION ON A BORROW REQUEST ============
-- decide_borrow_request() (migration 20260801220100) locks the row with
-- FOR UPDATE, which serializes concurrent callers so they can't corrupt
-- each other mid-transaction -- but it never checked the row's status once
-- the lock was acquired. A second approver (or a replayed/duplicated RPC
-- call) would simply re-read the already-decided row, ignore that it was
-- no longer 'pending', and write a second borrow_out/borrow_in pair,
-- doubling the transfer. QA test BR-11 caught this.
--
-- Adds the missing guard: once the lock is held, if the request is no
-- longer 'pending' the function raises instead of proceeding. Placed
-- immediately after the permission check and before the on-hand check /
-- any writes, so the second caller never touches inventory or the row.
--
